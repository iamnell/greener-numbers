...
/**
 * fetch-ev-data — Fetches NREL Alternative Fuels Data Center public EV station count
 * and upserts ev_data. Ported from supabase/functions/update-ev-data/index.ts
 * (Deno → Node.js for Railway Scheduled Tasks).
 *
...
try {
  ...
  const idResult = await db.from('cron_runs')
    .insert({ site: SITE, job_name: 'fetch-ev-data' })
    .select('id').single();
  ...
  const res = await fetch(
    `https://developer.nrel.gov/api/alt-fuel-stations/v1.json?${q}`,
  ...
  await db.from('ev_data').upsert(
    {
      site: SITE,
      dataset: 'public-electric-stations',
      geography: 'US',
      value: count,
      unit: 'stations',
      period,
  ...
  console.log(
    `OK ${JSON.stringify({ updated: 1, stations: count })}`
  );
} catch (e) {
  const msg = e.message || String(e);
  try { await db.from('cron_runs').update({ completed_at: new Date().toISOString(), status: 'failed', error_message: msg.slice(0, 2000) }).eq('id', idResult?.data?.id); } catch {} // best-effort
