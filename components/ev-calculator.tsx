...
type Mode = "charging" | "compare" | "home" | "offpeak";
...
const presets: Record<Mode, Values> = {
  ...
  offpeak: { annualMiles: 12000, efficiencyKwhPer100Miles: 28, chargingLossPercent: 10, peakRateCents: 28, offPeakRateCents: 14, offPeakPercent: 80 },
};
const fields: Record<Mode, Array<[string, string, string, number]>> = {
  ...
  offpeak: [["Annual miles driven", "annualMiles", "miles", 1], ["EV efficiency", "efficiencyKwhPer100Miles", "kWh per 100 miles", .1], ["Charging-loss assumption", "chargingLossPercent", "%", 1], ["Peak electricity rate", "peakRateCents", "¢ per kWh", .1], ["Off-peak electricity rate", "offPeakRateCents", "¢ per kWh", .1], ["Charging shifted off-peak", "offPeakPercent", "%", 1]],
};
...
export function EvCalculator({ mode, rateDefault }: { mode: Mode; rateDefault?: number }) {
  ...
  const result: any = useMemo(() => mode === "charging" ? calculateChargingCost(values) : mode === "compare" ? calculateEvVsGas(values) : mode === "home" ? calculateHomeCharger(values) : calculateOffPeakSavings(values), [mode, values]);
