...
export function calculateChargingCost(input) {
  ...
  const costPerMile = annualMiles > 0 ? combinedAnnualCost / annualMiles : 0;
  return { annualEnergyKwh: round(annualEnergyKwh), homeAnnualCost: round(homeAnnualCost), publicAnnualCost: round(publicAnnualCost), combinedAnnualCost: round(combinedAnnualCost), monthlyCost: round(combinedAnnualCost / 12), dailyCost: round(combinedAnnualCost / 365), costPerMile: round(costPerMile), costPer100Miles: round(costPerMile * 100), costPerFullCharge: round(batteryKwh * loss * homeRate) };
}
...
export function calculateEvVsGas(input) {
  ...
  const annualMiles = positive(input.annualMiles);
  const gasMpg = positive(input.gasMpg);
  const gasAnnualCost = gasMpg > 0 ? annualMiles / gasMpg * positive(input.gasPrice) : 0;
  const annualSavings = gasAnnualCost - ev.combinedAnnualCost;
  return { ...ev, evAnnualCost: ev.combinedAnnualCost, evMonthlyCost: ev.monthlyCost, gasAnnualCost: round(gasAnnualCost), gasMonthlyCost: round(gasAnnualCost / 12), gasCostPerMile: round(annualMiles > 0 ? gasAnnualCost / annualMiles : 0), annualSavings: round(annualSavings), monthlySavings: round(annualSavings / 12), fiveYearSavings: round(annualSavings * 5) };
}
...
export function calculateHomeCharger(input) {
  ...
  const enteredIncentives = [input.federalIncentive, input.stateIncentive, input.utilityRebate, input.manufacturerRebate, input.otherRebate].reduce((sum, value) => sum + positive(value), 0);
  const totalIncentives = Math.min(grossInstallationCost, enteredIncentives);
